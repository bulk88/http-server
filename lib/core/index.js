...
module.exports = function createMiddleware(_dir, _options) {
  ...
  chokidar.watch('.', {
    persistent: true,
    ignored: function(pathStr,statObj){
      if(watchIgnoreMatchCache[pathStr] !== undefined)
        ...
        /(?:^\..)|(?:^node_modules$)|(?:^include$)|(?:\.bak$)|(?:\.bat$)|(?:\.pl$)|(?:\.har$)|(?:\.yml$)/
        );
    },
    ignoreInitial: false,
    followSymlinks: true,
  ...
  }).on('add', function(pathStr, statObj){
  console.log('a '+pathStr);
  if(statObj.ctimeMs > appCacheNewestGlobalTimeStamp) {
    appCacheNewestGlobalTimeStamp = statObj.ctimeMs;
    console.log('update from watch');
  }
