...
const showDir = (function(){var f; return function(){if(!f){f=require('./show-dir')}return f.apply(null,arguments)}})();
...
const fallbackPaths = {
  "/": "/index.htm",
  "/stations.htm": "/sub/stations.htm",
  "/stop.htm": "/sub/stop.htm",
  "/stop_.htm": "/sub/stop_.htm",
  "/stop.appcache": "/sub/stop.appcache",
  "/tileMap.htm": "/sub/tileMap.htm",
  "/mapTileBackground.png": "/sub/mapTileBackground.png",
  "/transparentTile.gif": "/sub/transparentTile.gif",
  "/zoom_in.png": "/sub/zoom_in.png",
  "/zoom_in_over.png": "/sub/zoom_in_over.png",
  "/zoom_out.png": "/sub/zoom_out.png",
  "/zoom_out_over.png": "/sub/zoom_out_over.png",
};
...
module.exports = function createMiddleware(_dir, _options) {
  ...
  let appCacheNewestGlobalTimeStamp = 0;
  const watchIgnoreMatchCache = {};
  let watchInitialLoadReady = false;
...
//has closure var
function mkWatchCB (type) {
  return function (pathStr, statObj) {
    //reduce tests/branches on time critical initial start, so test twice
    //on the "1 file" post-start change detection (dev manual editing file)
    if (watchInitialLoadReady) {
      console.log(type + ' ' + pathStr);
      if (statObj.ctimeMs > appCacheNewestGlobalTimeStamp) {
        console.log(type + ' ctm update from watch');
      }
      if (statObj.mtimeMs > appCacheNewestGlobalTimeStamp) {
        console.log(type + ' mtm update from watch');
      }
    }
    if (statObj.ctimeMs > appCacheNewestGlobalTimeStamp) {
      appCacheNewestGlobalTimeStamp = statObj.ctimeMs;
    }
    if (statObj.mtimeMs > appCacheNewestGlobalTimeStamp) {
      appCacheNewestGlobalTimeStamp = statObj.mtimeMs;
    }
  };
  }
  //do not use *, * does NOT match "/" or "\" ever, see
  //https://github.com/micromatch/picomatch
  chokidar.watch('.', {
    persistent: true,
    ignored: function(pathStr /*,statObj unused, cant prevent in caller too*/){
      if(watchIgnoreMatchCache[pathStr] !== undefined)
        return watchIgnoreMatchCache[pathStr];
      else 
        return watchIgnoreMatchCache[pathStr] = !!pathStr.match(
        /(?:^\..)|(?:^node_modules$)|(?:^include$)|(?:^docs\/neighborhoodMaps$)|(?:^sub\/stationview_files$)|(?:^sub\/images$)|(?:\.bak$)|(?:\.bat$)|(?:\.pl$)|(?:\.har$)|(?:\.yml$)/
        );
    },
    //supress massive console noise on proc start
    ignoreInitial: true,
    followSymlinks: true,
    cwd: dir,
  ...
  }).on('change', mkWatchCB('change'))
    .on('add', mkWatchCB('add'))
    .on('ready', function(/*no args from caller*/){watchInitialLoadReady = true;console.log("Filemon ready");return;});
  ...
  return function middleware(req, res, next) {
    ...
    function serve(stat) {
      ...
      let etag = generateEtag(stat, weakEtags);
      let cacheControl = cache;
      let stream = null;
      const isAC = pathname.endsWith('.appcache');
      let acFileBody;

      if(isAC) {
        etag = etag.slice(0,-1)+"-"+(appCacheNewestGlobalTimeStamp|0)+'"';
        acFileBody = fs.readFileSync(file).toString('utf8');
        acFileBody = acFileBody.replace(/CACHE MANIFEST/, 'CACHE MANIFEST\n#'+(appCacheNewestGlobalTimeStamp|0));
        acFileBody = Buffer.from(acFileBody);
        stat.size = acFileBody.length;
      }

      if (!isAC && contentType && isTextFile(contentType)) {
      ...
      if(req.headers['x-fbpath']) {
        console.log("%s %s %s",'URL',chalk.cyan(pathname),'delivered thru fallback path table');
      }
      if(req.headers['x-fbdocs']) {
        console.log("%s %s %s",'URL',chalk.cyan(pathname),'delivered thru /docs fallback');
      }
        ...
        fs.stat(file, (err, stat) => {
          if (err && (err.code === 'ENOENT' || err.code === 'ENOTDIR')) {
            if (req.statusCode === 404) {
              ...
              status[404](res, next);
            } else if (!req.headers['x-fbpath'] && fallbackPaths[parsed.pathname]) {
              //anti-infinite loop if path mapping table is circular by accident
              req.headers['x-fbpath'] = "1";
              middleware({
                url: fallbackPaths[parsed.pathname]+(parsed.search ? parsed.search : ''),
                headers: req.headers,
              }, res, next);
            } else if (!req.headers['x-fbdocs'] && !fallbackPaths[parsed.pathname]) {
              //"/docs" last resort if no explicit path table match above
              req.headers['x-fbdocs'] = "1";
              middleware({
                url: ('/docs'+parsed.pathname)+(parsed.search ? parsed.search : ''),
                headers: req.headers,
              }, res, next);
